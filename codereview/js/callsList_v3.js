...
$(document).ready(function(){
	...
	calls.init({name:'loadStatesData',url:baseUrl+'/boomGetBreakupTimes.php',responseVarName:'loadStatesData',callback:callback.loadStatesData}).trigger({el:$('#loadStatesData')});	
	calls.init({name:'browserData',url:baseUrl+'/boomGetBrowserWiseLoadTimesAndPageViews.php',responseVarName:'browserData',callback:callback.browserData}).trigger({el:''});	
	calls.init({name:'loadTimePageView',url:baseUrl+'/boomGetLoadTimesAndPageViews.php',responseVarName:'loadTimePageView',callback:callback.loadTimePageViewData}).trigger({el:$('#visitor')});	
	calls.init({name:'loadTimeRanges',url:baseUrl+'/boomGetLoadTimeRanges.php',responseVarName:'loadTimeRanges',callback:Graph.timeChart}).trigger({el:$('#timeChart')});	
	calls.init({name:'platformData',url:baseUrl+'/boomGetOSWiseLoadTimesAndPageViews.php',responseVarName:'platformData',callback:Graph.platformData}).trigger({el:$('#browserShare')});	
	calls.init({name:'resourceTimer',url:baseUrl+'/boomGetCustomTimes.php',responseVarName:'resourceTimer',callback:Tables.resourceTimeTable}).trigger({el:$('#_')});		
